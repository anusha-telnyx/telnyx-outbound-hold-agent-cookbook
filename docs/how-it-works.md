...
This fake hotel is not a Telnyx AI Assistant. It is a TeXML test fixture generated by the local FastAPI app so users can test the outbound hold-agent workflow without needing a second company-owned AI agent. In the Telnyx portal, you should see the two outbound assistants only: IVR navigation and representative interaction.
